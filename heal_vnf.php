...
?>
<html>
<title>Heal VNF | CBAM REST Client</title>
...
if(isset($_POST['healing_cause'])){
    $HEALING_CAUSE=$_POST['healing_cause'];
}
...
echo "<b style='font-size:20px;'><a href='index.php'>Get CBAM Access Token</a></b>&nbsp;&nbsp;<object style='font-size:20px;'><select style='font-size:20px;' onchange='if (this.value) window.location.href=this.value'><option style='font-size:20px;' value='upload_package.php' selected>Upload VNF Package</option><option style='font-size:20px;' value='create_vnf.php'>Create VNF</option><option style='font-size:20px;' value='modify_extensions.php'>Modify Extensions</option><option style='font-size:20px;' value='instantiate_vnf.php'>Instantiate VNF</option><option style='font-size:20px;' value='scale_vnf.php'>Scale VNF</option><option style='font-size:20px;' value='heal_vnf.php' selected>Heal VNF</option><option style='font-size:20px;' value='execute_custom.php'>Execute Custom Operation</option><option style='font-size:20px;' value='terminate_vnf.php'>Terminate VNF</option><option style='font-size:20px;' value='cancel_operation.php'>Cancel Operation</option><option style='font-size:20px;' value='delete_vnf.php'>Delete VNF</option><option style='font-size:20px;' value='change_package_version.php'>Change Package Version</option><option style='font-size:20px;' value='delete_package.php'>Delete VNF Package</option><option style='font-size:20px;' value='list_packages.php'>List VNF Packages</option></select><br><br>";
...
echo "<br><br><input type='text' name='vnf_id' placeholder='VNF ID' style='display:inline; 
        position:relative; font-size:16px; padding:3px; border:1px solid #474747; background-color: #FFF; width:522px;' value='".$VNF_ID."'></input><br><br><input type='text' name='healing_cause' placeholder='Cause (Optional)' style='display:inline; 
        position:relative; font-size:16px; padding:3px; border:1px solid #474747;  background-color: #FFF; width:522px;' value='".$HEALING_CAUSE."'></input><br>";
...
echo "<input type='text' name='access_token' placeholder='Access Token' style='display:inline; 
        position:relative; font-size:16px; padding:3px; border:1px solid #474747; background-color: #FFF; width:428px;' value='".$ACCESS_TOKEN."'></input>&nbsp;<input type='submit' style='font-size:16px;' value='Heal VNF'></input></form>";


if(!empty($_POST['cbam_ip']) && !empty($_POST['access_token']) && !empty($_POST['vnf_id'])){
...
curl_setopt($ch, CURLOPT_URL, "https://$CBAM_IP/vnfm/lcm/v3/vnfs/$VNF_ID/heal");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
if(empty($_POST['key_a']) && empty($_POST['key_b']) && empty($_POST['key_c']) && empty($_POST['key_d']) && empty($_POST['key_e'])){
curl_setopt($ch, CURLOPT_POSTFIELDS, "{\"cause\": \"$HEALING_CAUSE\"}");
}
elseif (!empty($_POST['key_a']) && empty($_POST['key_b']) && empty($_POST['key_c']) && empty($_POST['key_d']) && empty($_POST['key_e'])){
$KEY_A=$_POST['key_a'];
$VALUE_A=$_POST['value_a'];
curl_setopt($ch, CURLOPT_POSTFIELDS, "{\"cause\": \"$HEALING_CAUSE\",\"additionalParams\":{\"$KEY_A\": \"$VALUE_A\"}}");
}
elseif (!empty($_POST['key_a']) && !empty($_POST['key_b']) && empty($_POST['key_c']) && empty($_POST['key_d']) && empty($_POST['key_e'])){
$KEY_A=$_POST['key_a'];
$VALUE_A=$_POST['value_a'];
$KEY_B=$_POST['key_b'];
$VALUE_B=$_POST['value_b'];
curl_setopt($ch, CURLOPT_POSTFIELDS, "{\"cause\": \"$HEALING_CAUSE\",\"additionalParams\":{\"$KEY_A\": \"$VALUE_A\", \"$KEY_B\": \"$VALUE_B\"}}");
}
elseif (!empty($_POST['key_a']) && !empty($_POST['key_b']) && !empty($_POST['key_c']) && empty($_POST['key_d']) && empty($_POST['key_e'])){
$KEY_A=$_POST['key_a'];
$VALUE_A=$_POST['value_a'];
...
$KEY_C=$_POST['key_c'];
$VALUE_C=$_POST['value_c'];
curl_setopt($ch, CURLOPT_POSTFIELDS, "{\"cause\": \"$HEALING_CAUSE\",\"additionalParams\":{\"$KEY_A\": \"$VALUE_A\", \"$KEY_B\": \"$VALUE_B\", \"$KEY_C\": \"$VALUE_C\"}}");
}
elseif (!empty($_POST['key_a']) && !empty($_POST['key_b']) && !empty($_POST['key_c']) && !empty($_POST['key_d']) && empty($_POST['key_e'])){
$KEY_A=$_POST['key_a'];
$VALUE_A=$_POST['value_a'];
...
$KEY_D=$_POST['key_d'];
$VALUE_D=$_POST['value_d'];
curl_setopt($ch, CURLOPT_POSTFIELDS, "{\"cause\": \"$HEALING_CAUSE\",\"additionalParams\":{\"$KEY_A\": \"$VALUE_A\", \"$KEY_B\": \"$VALUE_B\", \"$KEY_C\": \"$VALUE_C\", \"$KEY_D\": \"$VALUE_D\"}}");
}
else {
$KEY_A=$_POST['key_a'];
$VALUE_A=$_POST['value_a'];
...
$KEY_E=$_POST['key_e'];
$VALUE_E=$_POST['value_e'];
curl_setopt($ch, CURLOPT_POSTFIELDS, "{\"cause\": \"$HEALING_CAUSE\",\"additionalParams\":{\"$KEY_A\": \"$VALUE_A\", \"$KEY_B\": \"$VALUE_B\", \"$KEY_C\": \"$VALUE_C\", \"$KEY_D\": \"$VALUE_D\", \"$KEY_E\": \"$VALUE_E\"}}");
}
