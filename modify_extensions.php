...
?>
<html>
<title>Modify Extensions | CBAM REST Client</title>
...
echo "<object style='font-size:20px;'>Parameter File (JSON):</object><br><br><input type='file' name='parameter_file' id='parameter_file' placeholder='Parameter File' style='display:inline; position:relative; padding:3px; border:1px solid #474747; font-size:16px; background-color: #FFF; width:536px;' value='".$PARAMETER_FILE."'></input><br><br>";

echo "<input type='text' name='access_token' placeholder='Access Token' style='display:inline; position:relative; padding:3px; border:1px solid #474747; font-size:16px; background-color: #FFF; width:384px;' value='".$ACCESS_TOKEN."'></input>&nbsp;<input type='submit' style='font-size:16px;' value='Modify Extensions'></input></form>";

if(!empty($_POST['vnf_id']) && isset($_FILES["parameter_file"]["name"]) && !empty($_POST['access_token']) && !empty($_POST['cbam_ip'])){
$ACCESS_TOKEN=$_POST['access_token'];
$CBAM_IP=$_POST['cbam_ip'];
$VNF_ID=$_POST['vnf_id'];

$result = shell_exec('curl -k -XPATCH -i -H "Content-Type: application/json; charset=UTF-8" -H "Authorization: bearer '.$ACCESS_TOKEN.'" -H "Nokia-VNFM-API-Version: 3.3" --data-binary @"parameter_files/'.$PARAMETER_FILE.'" https://'.$CBAM_IP.'/vnfm/lcm/v3/vnfs/'.$VNF_ID.'');
